...
| Pending commands | `DeviceSession` | 8 | reject new request; each request has timeout/cancellation | all retired on disconnect/generation change |
...
Connection generations prevent responses from a retired session from satisfying requests in a later session. Cancellation distinguishes expected application shutdown from unexpected failure. UI work is marshalled explicitly through the WPF dispatcher; the acquisition path never performs chart rendering or file I/O.
