...
4. Concurrency/policy tests: bounded drop-oldest behavior, request timeout, cancellation, duplicate-response policy, and deterministic shutdown paths.
5. Simulator integration tests: handshake, idle/streaming transitions, stream start/stop, CRC corruption, sample loss, and command failure behavior.
...
passing evidence by undocumented retry.
